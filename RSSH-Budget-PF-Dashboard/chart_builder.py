...
def build_main_chart(app, region, country, ip, component):
    ...
    scope_b, scope_i, _ = filter_data(region=region, country=country, component=component)
    ...
    custom_data_b = []
    for y_m in y_vals:
        int_agg = b_filt[b_filt['Module'] == y_m].groupby('Intervention')['Total Amount'].sum().reset_index()
        ...
        if int_agg.empty or int_agg['Total Amount'].sum() == 0:
            ...
            data = [{'Intervention': str(r['Intervention']), 'Amount': f"{r['Total Amount']/1e6:,.1f}"}
        ...
        for is_custom in [False, True]:
            ...
            for i, y_m in enumerate(y_vals):
                ...
                if c > 0:
                    ...
                    if is_custom:
                        grouped = (mod_sub.groupby(['IndicatorCustomName', '__sort_order', '__sort_code'], dropna=False)
                        ...
                        grouped = (mod_sub.groupby(['IndicatorCode', 'IndicatorDescription', '__sort_order', '__sort_code'], dropna=False)
                                   .size().reset_index(name='count'))
                        grouped.sort_values(by=['count', '__sort_order', '__sort_code'],
                                            ascending=[False, True, True], inplace=True)
