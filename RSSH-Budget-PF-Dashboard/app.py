...
def display_hover(hoverData):
    ...
    if type_ == 'BUDGET':
        ...
        rows = [html.Tr([html.Th("Intervention", style=style_th), html.Th("Budget ($M)", style=budget_th)])]
        rows += [html.Tr([html.Td(d['Intervention'], style=style_td),
                          html.Td(d['Amount'], style=budget_td)]) for d in data]
        ...
        rows += [html.Tr([html.Td(d['name'], style=style_td),
                          html.Td(d.get('count', ''), style=style_no_wrap)]) for d in data]
        return True, bbox, html.Div([
        ...
        title = obj.get('title', 'Indicators')
        rows = [html.Tr([html.Th("Indicator Code", style=style_th_no_wrap),
                         html.Th("Indicator Name", style=style_th),
                         html.Th("Count", style=style_th_no_wrap)])]
        rows += [html.Tr([html.Td(d['code'], style=style_no_wrap),
                          html.Td(d['desc'], style=style_td),
                          html.Td(d.get('count', ''), style=style_no_wrap)]) for d in data]
        return True, bbox, html.Div([
            html.B(title, style={'marginBottom': '10px', 'display': 'block'}),
            html.Table(rows, style=style_table)
        ...
        rows += [html.Tr([html.Td(d['ip'], style=style_no_wrap),
                          html.Td(d['act'], style=style_td)]) for d in data]
        return True, bbox, html.Div([
...
def update_overview_chart(region, custom_wptm, merge, percent, excl_pm, sort_b):
