...
def get_equity_data(iso3, filepath):
    try:
        df = pd.read_excel(filepath, sheet_name='WIQ')
...
def get_precalculated_stats(iso3, indicator_name):
    try:
        ...
        df_stats = pd.read_csv('data/peer_stats_summary.csv')
    except Exception as e:
...
def get_oop_peer_lines(iso3="NGA"):
    try:
        ...
        oop = pd.read_excel('data/OOP.xlsx')
...
def get_md_data(iso3="NGA"):
    try:
        df_md = pd.read_excel('data/MD_per10k.xlsx')
...
def get_nurse_data(iso3="NGA"):
    try:
        df_nurse = pd.read_excel('data/Nurse_per10k.xlsx')
...
def get_chw_data(iso3="NGA"):
    try:
        df_chw = pd.read_excel('data/CHW_per10k.xlsx')
